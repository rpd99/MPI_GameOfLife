...
void define_work_portions(int grid_rows, int grid_cols, struct work_portion portions[], int n_portions) {
    ...
    portions[n_portions - 1].n_rows += remainder_rows;  // assegna le righe restanti all'ultima porzione
}

int count_alive_neighbours(int grid_rows, int grid_cols, char (*curr_grid)[][grid_cols], int r, int c) {
    int alive_neighbours = 0;                                // conta i vicini ALIVE
    for (int row = r - 1; row <= r + 1; row++) {      // righe sopra-corrente-sotto
        for (int col = c - 1; col <= c + 1; col++) {  // colonne sinistra-corrente-destra
            if (!(row == r && col == c) &&             // non contare la cella stessa come aliveNeighbour
                (row >= 0 && col >= 0) &&                  // check bound superiore/sinistro griglia
                (row < grid_rows && col < grid_cols) &&    // check bound inferiore/destro griglia
                ((*curr_grid)[row][col] == ALIVE)) {
                alive_neighbours++;
            }
        }
    }
    ...
    if (rank == MASTER) {
        ...
        for (int iter = 0; iter < n_iters; iter++) {
            ...
            if (iter < n_iters - 1) {
                ...
                if (portion.has_top_ghost) {
                    MPI_Irecv(&(*next_grid)[top_ghost_row_index][0], tot_cols, MPI_CHAR, rank - 1, iter, MPI_COMM_WORLD, &neigh_rcv_reqs[n_neigh_rcv++]);
                    MPI_Isend(&(*next_grid)[top_ghost_row_index + 1][0], tot_cols, MPI_CHAR, rank - 1, iter, MPI_COMM_WORLD, &neigh_snd_reqs[n_neigh_snd++]);
                }
                if (portion.has_bottom_ghost) {
                    MPI_Irecv(&(*next_grid)[bot_ghost_row_index][0], tot_cols, MPI_CHAR, rank + 1, iter, MPI_COMM_WORLD, &neigh_rcv_reqs[n_neigh_rcv++]);
                    MPI_Isend(&(*next_grid)[bot_ghost_row_index - 1][0], tot_cols, MPI_CHAR, rank + 1, iter, MPI_COMM_WORLD, &neigh_snd_reqs[n_neigh_snd++]);
                }
